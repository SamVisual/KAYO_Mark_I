...
export default function SettingsView({ apiKey, setApiKey, onSaved }) {
  ...
  // Ref so the auto-navigate timer can be cancelled on unmount (BUG #7).
  ...
  // Clear the auto-navigate timer if the component unmounts before it fires (BUG #7).
  useEffect(() => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      savedTimerRef.current = setTimeout(() => onSaved?.(), 800)
    } catch (err) {
    ...
    <div className="flex flex-col h-full overflow-y-auto p-8 anim-fade-up">
      <div className="max-w-lg w-full mx-auto flex flex-col gap-8">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Settings size={18} style={{ color: 'rgba(0,212,255,0.7)' }} />
          </div>
          <div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <p className="text-xs" style={{ color: 'rgba(255,255,255,0.25)' }}>
            ...
            Erhältlich auf{' '}
            <span style={{ color: 'rgba(0,212,255,0.6)' }}>console.anthropic.com</span>
          </p>
          ...
          <button
            ...
            style={{ background: 'linear-gradient(135deg, #00d4ff, #0088ff)', color: '#000' }}
          >
